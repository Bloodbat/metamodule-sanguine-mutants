...
struct Temulenti : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		if (stMode.process(static_cast<bool>(std::round(params[PARAM_MODE].getValue())))) {
			mode = bumps::GeneratorMode((static_cast<int>(mode) + 1) % 3);
			generator.set_mode(mode);
		}

		bumps::GeneratorRange range = generator.range();
		if (stRange.process(static_cast<int>(std::round(params[PARAM_RANGE].getValue())))) {
			range = bumps::GeneratorRange((static_cast<int>(range) - 1 + 3) % 3);
			generator.set_range(range);
		}

		bool bSync = static_cast<bool>(std::round(params[PARAM_SYNC].getValue()));
		...
		if (generator.writable_block()) {
			...
			if ((quantize = static_cast<int>(std::round(params[PARAM_QUANTIZER].getValue())))) {
		...
		if (bLightsTurn) {
			...
			generator.feature_mode_ = bumps::Generator::FeatureMode(static_cast<int>(std::round(params[PARAM_MODEL].getValue())));
			...
			displayModel = temulentiDisplayModels[static_cast<int>(std::round(params[PARAM_MODEL].getValue()))];
