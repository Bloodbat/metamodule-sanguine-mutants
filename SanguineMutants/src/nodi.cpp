...
struct Nodi : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		bVCAEnabled = params[PARAM_VCA].getValue() >= 0.5f;
		bDritfEnabled = params[PARAM_DRIFT].getValue() >= 0.5f;
		bFlattenEnabled = params[PARAM_FLAT].getValue() >= 0.5f;
		bSignatureEnabled = params[PARAM_SIGN].getValue() >= 0.5f;
		bAutoTrigger = params[PARAM_AUTO].getValue() >= 0.5f;
		bPaques = params[PARAM_MORSE].getValue() >= 0.5f;
