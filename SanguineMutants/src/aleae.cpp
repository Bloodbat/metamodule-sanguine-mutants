...
struct Aleae : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		for (int section = 0; section < kMaxModuleSections; section++) {
			...
			rollModes[section] = static_cast<RollModes>(std::round(params[PARAM_ROLL_MODE_1 + section].getValue()));
			outModes[section] = static_cast<OutModes>(std::round(params[PARAM_OUT_MODE_1 + section].getValue()));
