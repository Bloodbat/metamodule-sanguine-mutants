...
struct Etesia : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		if (outputBuffer.empty()) {
			...
			etesiaProcessor->set_num_channels(static_cast<bool>(std::round(params[PARAM_STEREO].getValue())) ? 2 : 1);
			etesiaProcessor->set_low_fidelity(!static_cast<bool>(std::round(params[PARAM_HI_FI].getValue())));
