...
struct Nebulae : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		if (outputBuffer.empty()) {
			...
			cloudsProcessor->set_num_channels(static_cast<bool>(std::round(params[PARAM_STEREO].getValue())) ? 2 : 1);
			cloudsProcessor->set_low_fidelity(!static_cast<bool>(std::round(params[PARAM_HI_FI].getValue())));
