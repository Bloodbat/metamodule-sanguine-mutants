...
struct Vimina : SanguineModule {
	...
	*/
	...
	void process(const ProcessArgs& args) override {
		...
		for (int section = 0; section < kMaxModuleSections; section++) {
			resetButtons[section] = btReset[section].process(static_cast<bool>(std::round(params[PARAM_RESET_1 + section].getValue())));
		}
		...
		for (int channel = 0; channel < channelCount; channel++) {
			...
			for (uint8_t section = 0; section < kMaxModuleSections; section++) {
				channelFunction[section] = SectionFunctions(static_cast<int>(std::round(params[PARAM_MODE_1 + section].getValue())));
