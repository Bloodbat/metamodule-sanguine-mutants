...
struct Fluctus : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		if (outputBuffer.empty()) {
			...
			fluctusProcessor->set_num_channels(static_cast<bool>(std::round(params[PARAM_STEREO].getValue())) ? 2 : 1);
			fluctusProcessor->set_low_fidelity(!static_cast<bool>(std::round(params[PARAM_HI_FI].getValue())));
			fluctusProcessor->Prepare();

			bool frozen = params[PARAM_FREEZE].getValue() >= 0.5f;
