...
struct Anuli : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		polyphonyMode = static_cast<int>(std::round(params[PARAM_POLYPHONY].getValue()));
		fxModel = static_cast<rings::FxType>(std::round(params[PARAM_FX].getValue()));

		int modeNum = static_cast<int>(std::round(params[PARAM_MODE].getValue()));
