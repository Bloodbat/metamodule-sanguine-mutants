...
struct Distortiones : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		if (btModeSwitch.process(std::round(params[PARAM_MODE_SWITCH].getValue()))) {
		...
		for (int channel = 0; channel < channelCount; channel++) {
			...
			distortionesParameters[channel]->carrier_shape = static_cast<int32_t>(std::round(params[PARAM_CARRIER].getValue()));
