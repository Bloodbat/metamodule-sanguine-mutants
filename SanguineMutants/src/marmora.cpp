...
struct Marmora : SanguineModule {
	...
	void process(const ProcessArgs& args) override {
		...
		// Lock modes
		dejaVuLockModeT = static_cast<int>(std::round(params[PARAM_T_SUPER_LOCK].getValue())) ? DEJA_VU_SUPER_LOCK : DEJA_VU_LOCK_OFF;
		dejaVuLockModeX = static_cast<int>(std::round(params[PARAM_X_SUPER_LOCK].getValue())) ? DEJA_VU_SUPER_LOCK : DEJA_VU_LOCK_OFF;
		...
		bDejaVuTEnabled = static_cast<bool>(std::round(params[PARAM_DEJA_VU_T].getValue()));
		bDejaVuXEnabled = static_cast<bool>(std::round(params[PARAM_DEJA_VU_X].getValue()));

		xScale = static_cast<int>(std::round(params[PARAM_SCALE].getValue()));
		xClockSourceInternal = static_cast<int>(std::round(params[PARAM_INTERNAL_X_CLOCK_SOURCE].getValue()));
	...
	void stepBlock() {
		...
		tGenerator.set_model(static_cast<marbles::TGeneratorModel>(std::round(params[PARAM_T_MODE].getValue())));
		tGenerator.set_range(static_cast<marbles::TGeneratorRange>(std::round(params[PARAM_T_RANGE].getValue())));
		...
		if (!bScaleEditMode) {
			...
			x.control_mode = static_cast<marbles::ControlMode>(std::round(params[PARAM_X_MODE].getValue()));
			x.voltage_range = static_cast<marbles::VoltageRange>(std::round(params[PARAM_X_RANGE].getValue()));
			// TODO Fix the scaling
			//  I think the double multiplication by 0.5f (both in the next line and when assigning "u" might be wrong: custom scales seem to behave nicely when NOT doing that...) -Bat Ed.
			...
			x.register_mode = static_cast<bool>(std::round(params[PARAM_EXTERNAL].getValue()));
			x.register_value = u;
